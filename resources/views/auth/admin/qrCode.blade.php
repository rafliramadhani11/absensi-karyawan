<x-app-layout>
    ...
    @if ($date->format('H') >= 8 && $date->format('H') <= 22)
        <div class="bg-white rounded-md shadow-md p-6 col-span-2">
            <div class="sm:flex sm:items-center sm:justify-around mb-4">
                <div class="flex items-center justify-center ">
                    <div>
                        {{ QrCode::size(160)->margin(2)->generate(route('user.hadir')) }}
                        <p class="text-center text-sm text-emerald-600 font-semibold">*Scan Absen Hadir</p>
                    </div>
                </div>
                <div class="flex items-center justify-center ">
                    <div>
                        {{ QrCode::size(160)->margin(2)->generate(route('user.pulang')) }}
